...
const GAME_CONFIG = Object.freeze({
  title: 'Asfaltit Kuyuları olan illeri bul',
  menuTitle: "Asfaltit Kuyuları",
  taskText: 'Asfaltit Kuyuları olan illeri bul',
  learnTitle: 'Asfaltit Kuyuları olan illeri bul',
  finishTitle: 'Tüm yerler tamamlandı',
  phaseTransitionText: 'Çıkarılan yerler tamamlandı. Şimdi asfaltitin işlendiği illeri bul.',
  ...
  phases: Object.freeze([
    Object.freeze({
      id: 'extraction',
      taskText: 'Asfaltit Kuyuları olan illeri bul',
      markerVariant: 'extraction',
      mineData: Object.freeze([
        { label: "", cityId: "hakkari", x: 938.18, y: 316.43 },
        { label: "", cityId: "sirnak", x: 872.13, y: 322.29 },
        
      ])
    }),
    Object.freeze({
      id: 'processing',
      taskText: 'Asfaltit Santrali olan illeri bul.',
      markerVariant: 'processing',
      mineData: Object.freeze([
        // Geçici iğne konumları; daha sonra harita üstünde hassaslaştırılabilir.
        { label: "Silopi", cityId: "sirnak", x: 849.22, y: 335.08 }
      ])
    })
  ])
});

const GAME_PHASES = GAME_CONFIG.phases;
let activePhaseIndex = 0;
let MINE_DATA = GAME_PHASES[activePhaseIndex].mineData;
let TASK_TEXT = GAME_PHASES[activePhaseIndex].taskText;
...
function getActivePhase() {
    return GAME_PHASES[activePhaseIndex] || GAME_PHASES[0];
}

function syncActivePhaseData() {
    const activePhase = getActivePhase();
    MINE_DATA = activePhase.mineData;
    TASK_TEXT = activePhase.taskText || GAME_CONFIG.taskText;
}
...
function hasNextPhase() {
    return activePhaseIndex < GAME_PHASES.length - 1;
}
...
function startNextPhase() {
    ...
    showStatusToast(GAME_CONFIG.phaseTransitionText, 'success');
    return true;
}
...
function buildMineLookup() {
    ...
    MINE_DATA.forEach((mine, index) => {
        ...
        const markerVariant = mine.markerVariant || activePhase.markerVariant || activePhase.id;
        const labelOffset = markerVariant === 'processing'
        ...
        cityMines.push({
            ...
            markerVariant,
            mineKey: `${activePhase.id}-${mine.cityId}-${index}`,
            labelDx: mine.labelDx ?? labelOffset.dx,
            labelDy: mine.labelDy ?? labelOffset.dy,
            labelAnchor: mine.labelAnchor || labelOffset.anchor
        });
    });
}
...
function queueFinishGameIfCompleted() {
    ...
    setTimeout(() => {
        if (!startNextPhase()) finishGame();
    }, UI_TIMINGS.finishDelayMs);
}
...
function createMineMarker(mine) {
    ...
    const markerVariant = mine.markerVariant || 'extraction';
    group.setAttribute('class', `mine-marker mine-marker--${markerVariant}`);
    group.setAttribute('data-phase-id', mine.phaseId || getActivePhase().id);
    ...
    if (markerVariant === 'processing') {
        const factory = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        factory.setAttribute('class', 'mine-factory-icon');
        ...
        factory.setAttribute('transform', `translate(${mine.x - 7.2} ${mine.y - 6.3}) scale(0.0225)`);
        markerParts.push(factory);
    } else {
    ...
    group.append(glow, ...markerParts, labelText);
